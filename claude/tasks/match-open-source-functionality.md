...
export const api = {
  ...
  getAuthor: (slug: string) => fetchAPI(`/authors/${slug}`),
  ...
  getSkillTree: (skillId: string) => fetchAPI(`/skills/${skillId}/tree`),
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-gray-600 mb-8">
        Browse skills by author. {pagination?.total || 0} authors have
        contributed skills.
      </p>
  ...
  return (
    <div className="p-6">
      ...
      <h2 className="text-xl font-semibold mb-4">
        Skills by {author.name || author.slug}
      </h2>
...
export default function SkillDetail() {
  ...
  const skillPath =
    skill.source_url?.split('/tree/main/')?.[1] || `skills/${skill.slug}`

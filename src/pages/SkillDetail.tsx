...
import { Star, GitFork, Copy, Check, Download, ExternalLink } from 'lucide-react';
    ...
    <div className="bg-slate-50 min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 mb-6">
          ...
          <div className="lg:col-span-3 bg-white rounded-xl p-6 border border-slate-200/80 shadow-sm shadow-slate-100">
            <div className="flex items-start justify-between gap-4 mb-4">
              <h1 className="text-xl font-semibold text-slate-900 tracking-tight">{skill.name}</h1>
              <button
            ...
            <p className="text-slate-500 text-[14px] leading-relaxed mb-5">{skill.description}</p>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm shadow-slate-100 flex-1">
              <h3 className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-4">Author</h3>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[14px] font-semibold text-slate-800">{skill.author_name || skill.repo_owner}</p>
                  {skill.author_url && (
                    <a
          ...
          <div className="lg:col-span-3 bg-white rounded-xl p-6 border border-slate-200/80 shadow-sm shadow-slate-100">
            <h2 className="text-[15px] font-semibold text-slate-800 mb-4 tracking-tight">Install this agent skill</h2>
          ...
          {skill.repo_owner && skill.repo_name && (
            <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm shadow-slate-100">
              <h3 className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-4">Repository</h3>
              <a
                href={repoUrl}
                target="_blank"
                rel="noopener noreferrer"
              ...
              <div className="flex items-center gap-4 mt-4 pt-4 border-t border-slate-100">
                {skill.repo_stars !== undefined && (
                  <div className="flex items-center gap-1.5 text-[12px] text-slate-500">
                    ...
                    <span className="font-medium">{skill.repo_stars.toLocaleString()}</span>
                  </div>
                )}
                {skill.repo_forks !== undefined && (
                  <div className="flex items-center gap-1.5 text-[12px] text-slate-500">
                    <GitFork size={12} />
                    <span className="font-medium">{skill.repo_forks.toLocaleString()}</span>
                  </div>
                )}
              </div>
              ...
              {skill.repo_license && (
                <div className="mt-4 pt-4 border-t border-slate-100">
                  ...
                  <span className="text-[11px] font-medium text-slate-600">{skill.repo_license}</span>
                </div>
              )}
            </div>
          )}
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-6 shadow-sm shadow-slate-100">
          <h2 className="text-[15px] font-semibold text-slate-800 mb-1 tracking-tight">Skill Files</h2>
          <p className="text-[13px] text-slate-400 mb-5">Browse the folder contents for {skill.name}</p>

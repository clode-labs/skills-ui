...
export const authApi = {
  ...
  getSkills: (params?: Record<string, string>) => {
    const query = params ? '?' + new URLSearchParams(params).toString() : '';
    return fetchAPI<SkillListResponse>(`/api/v1/skills${query}`, { requiresAuth: true });
  },

  searchSkills: (query: string, page = 1, limit = 20) => {
    const params = new URLSearchParams({ q: query, page: page.toString(), limit: limit.toString() });
    return fetchAPI<SkillListResponse>(`/api/v1/skills/search?${params}`, { requiresAuth: true });
  },

  getFeaturedSkills: () => {
    return fetchAPI<SkillListResponse>('/api/v1/skills/featured', { requiresAuth: true });
  },

  getSkill: (owner: string, repo: string, name: string) => {
    return fetchAPI<SkillWithVersionResponse>(`/api/v1/skills/${owner}/${repo}/${name}`, { requiresAuth: true });
  },
  ...
  getCategories: () => {
    return fetchAPI<CategoryListResponse>('/api/v1/categories', { requiresAuth: true });
  },

  // Tags (includes private skill counts for user)
  getTags: (page = 1, limit = 100) => {
    return fetchAPI<TagListResponse>(`/api/v1/tags?page=${page}&limit=${limit}`, { requiresAuth: true });
  },

  // User profile
  getProfile: () => {
    return fetchAPI<{ user: { id: string; email: string; name: string } }>('/api/v1/me', { requiresAuth: true });
  },
};

...
export const api = {
  ...
  getSkills: (params?: {
    ...
    return fetchAPI<SkillListResponse>(`/skills${queryString ? `?${queryString}` : ''}`);
  },
  ...
  getMySkills: async (params?: { page?: number; limit?: number }) => {
    ...
    const queryString = query.toString();
    const response = await fetchAPI<SkillListResponse>(`/skills${queryString ? `?${queryString}` : ''}`);
  ...
  createSkill: async (data: CreateSkillRequest): Promise<CreateSkillResponse> => {
  ...
  updateSkill: async (skillId: string, data: { name?: string; description?: string; tags?: string[] }): Promise<SkillResponse> => {
  ...
  getSkillVersion: (skillId: string, version: string) => {
    return fetchAPI<SkillVersionResponse>(`/skills/${skillId}/versions/${version}`);
  },
